...
(function() {
    ...
    this.afterGetProperties = function(e, path, newProperties) {
        ...
        var self = this;
        Async.forEach(Object.keys(newProperties["404"]), function(propName, next) {
            var val = false;
            switch (propName) {
    ...
    this.getLocks = function(uri, returnChildLocks, cbgetlocks) {
        ...
        Async.forEach(uri.split("/"), function(uriPart, next) {
            if (currentPath)
                currentPath += "/";
            currentPath += uriPart;
    ...
    this.validateLock = function(urls, checkChildLocks, cbvalidate) {
        ...
        Async.forEach(urls, function(url, next) {
            self.getLocks(url, false, function(err, aLocks) {
                ...
                Async.forEach(conditions, function(condition, next2) {
                    var conditionUri;
                    try {
                    ...
                    Async.forEach(condition["tokens"], function(conditionToken, next3) {
                        var etagValid = true;
                        var lockValid = true;
