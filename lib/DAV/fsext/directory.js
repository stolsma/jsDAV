...
var jsDAV_FSExt_Node  = require("./node").jsDAV_FSExt_Node;
var jsDAV_FSExt_File  = require("./file").jsDAV_FSExt_File;
...
function jsDAV_FSExt_Directory(path) {
    this.path = path;
}

exports.jsDAV_FSExt_Directory = jsDAV_FSExt_Directory;
    ...
    this.getChild = function(name, cbfsgetchild) {
        ...
        Fs.stat(path, function(err, stat) {
            ...
            cbfsgetchild(null, stat.isDirectory()
                ? new jsDAV_FSExt_Directory(path)
                : new jsDAV_FSExt_File(path))
        });
    };
        ...
        Async.readdir(this.path)
             ...
             .each(function(file, cbnextdirch) {
                 nodes.push(file.stat.isDirectory()
                     ? new jsDAV_FSExt_Directory(file.path)
                     : new jsDAV_FSExt_File(file.path)
                 );
                 cbnextdirch();
             })
             .end(function() {
...
}).call(jsDAV_FSExt_Directory.prototype = new jsDAV_FSExt_Node());
