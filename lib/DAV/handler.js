...
function jsDAV_Handler(server, req, resp) {
    ...
    for (var plugin in server.plugins) {
        if (!server.plugins.hasOwnProperty(plugin))
            continue;
...
(function() {
        ...
        this.getRequestBody("utf8", function(err, data) {
            ...
            self.parsePropfindRequest(data, function(err, requestedProperties) {
                ...
                self.getPropertiesForPath(path, requestedProperties, depth, function(err, newProperties) {
                    ...
                    console.log(newProperties,"resulting XML:",self.generateMultiStatus(newProperties));
    ...
    this.httpMkcol = function() {
        ...
        this.getRequestBody("utf8", function(err, requestBody) {
            if (requestBody) {
                ...
                XmlUtil.loadDOMDocument(requestBody, function(err, dom) {
                    var firstChild = dom.firstChild;
                    if (XmlUtil.toClarkNotation(firstChild) !== "{DAV:}mkcol") {
                    ...
                    for (; i < l; ++i) {
                        ...
                        if (XmlUtil.toClarkNotation(childNode) !== "{DAV:}set")
                            continue;
                        properties = Util.extend(properties, XmlUtil.parseProperties(childNode, self.propertyMap));
                    }
                    ...
                    for (i = 0, c = rtNode.childNodes, l = c.length; i < l; ++i)
                        resourceType.push(XmlUtil.toClarkNotation(c[i]));
    ...
    this.httpReport = function() {
        var self = this;
        this.getRequestBody("utf8", function(err, data) {
            XmlUtil.loadDOMDocument(data, function(err, dom) {
                var reportName = XmlUtil.toClarkNotation(dom);
    ...
    this.generateMultiStatus = function(fileProperties) {
        ...
        for (namespace in this.xmlNamespaces) {
            if (!this.xmlNamespaces.hasOwnProperty(namespace))
                continue;
        ...
        for (var i in fileProperties) {
            if (!fileProperties.hasOwnProperty(i))
                continue;
                         ...
                         function parseProperties() {
                             if (allProperties) {
                                 ...
                                 for (var i in newProperties["200"]) {
                                     if (!newProperties["200"].hasOwnProperty(i))
                                        continue;
                                     keys.push(i);
                                 }
    ...
    this.parsePropfindRequest = function(body, cbpropfindreq) {
        ...
        XmlUtil.loadDOMDocument(body, function(err, oXml) {
            //Util.log("XML ", oXml);
            if (!Util.empty(err))
                return cbpropfindreq(err);
            cbpropfindreq(null, Object.keys(XmlUtil.parseProperties(oXml.propfind || oXml)));
        });
    };
    ...
    this.parseProppatchRequest = function(body, cbproppatchreq) {
        ...
        var self = this;
        XmlUtil.loadDOMDocument(body, function(err, dom) {
            if (!Util.empty(err))
            ...
            for (; i < l; ++i) {
                ...
                innerProperties = XmlUtil.parseProperties(child, self.propertyMap);
                for (propertyName in innerProperties) {
                    if (!innerProperties.hasOwnProperty(propertyName))
                        continue;
    ...
    this.updateProperties = function(uri, properties, cbupdateprops) {
        ...
        this.server.tree.getNodeForPath(uri, function(err, node) {
            ...
            if (!hasError) {
                for (propertyName in properties) {
                    if (!properties.hasOwnProperty(propertyName) || self.protectedProperties.indexOf(propertyName) == -1)
                        continue;
                    Util.arrayRemove(result[uri]["403"], propertyName);
            ...
            function finish() {
                ...
                for (propertyName in remainingProperties) {
                    if (!remainingProperties.hasOwnProperty(propertyName))
                        continue;
                ...
                for (var status in result[uri]) {
                    if (!result[uri].hasOwnProperty(status))
                        continue;
